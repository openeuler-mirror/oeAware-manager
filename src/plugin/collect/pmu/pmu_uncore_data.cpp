...
oeaware::Register<PmuUncoreData> PmuUncoreData::pmuUncoreReg("pmu_uncore_collector");

void PmuUncoreData::Serialize(oeaware::OutStream &out) const
{
    out << interval << len;
    for (int i = 0; i < len; i++) {
        int count = 0;
...
void PmuUncoreData::Deserialize(oeaware::InStream &in)
{
    in >> interval >> len;
