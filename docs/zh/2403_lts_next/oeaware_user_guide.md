...
配置文件路径: /etc/oeAware/plugin/disk_adapt_ic.yaml

配置说明：

```yaml
iops_threshold: 1000000          # IOPS 阈值，超过此值开启中断聚合，范围 0-10000000
sample_period_ms: 100              # 采样周期（毫秒），范围 100-1000
interrupt_coalescing_value: "0x102"  # 中断聚合寄存器值，十六进制字符串，默认 0x102（Enable + Time+Count 模式）
iops_4k_ratio_threshold: 0.5       # 4K 块占比阈值，用于判断是否为 4K 对齐 IO，范围 (0, 1.0]
```

配置示例：

```yaml
iops_threshold: 500000
sample_period_ms: 200
interrupt_coalescing_value: "0x102"
iops_4k_ratio_threshold: 0.3
```

注意：

1. `iops_threshold` 建议根据业务场景调整，高负载场景可适当调高
2. `interrupt_coalescing_value` 为 NVMe Controller Register (CC) 中 `INT_COALESCE` 字段值，参考 NVMe 规范
3. 配置文件不存在时使用默认值，无需额外配置即可启用
4. 依赖 `nvme-cli` 工具（`yum install nvme-cli`）
